    ...
    <div className="min-h-screen bg-[#001a1a]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center justify-center text-center space-y-12 mb-24">
            ...
            <h1 className="text-7xl lg:text-[9rem] font-black tracking-[-0.07em] text-white leading-none uppercase text-glow">
              APPAREL <span className="text-cyan-400">SHOPPING CART.</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-4 bg-white/5 p-4 rounded-[3rem] border border-white/10">
              {(Object.keys(APPAREL_DATA) as SubCategory[]).map(tab => (
                <button 
                  ...
                  className={`px-12 py-5 rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] transition-all ${activeTab === tab ? 'bg-cyan-500 text-[#001a1a] shadow-[0_20px_40px_-10px_rgba(0,242,255,0.4)]' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
                >
                  {tab}
                </button>
              ))}
              ...
              >
                ...
                <div className="p-12 space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-cyan-600 uppercase tracking-widest">{item.styles?.[0] || 'Standard'} Fit</span>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-8">
           ...
           <div className="relative z-10 w-full max-w-5xl bg-white rounded-[5rem] overflow-hidden shadow-[0_50px_150px_-30px_rgba(0,242,255,0.3)] flex flex-col lg:flex-row animate-in slide-in-from-bottom-20 duration-700">
              ...
              <div className="flex-1 p-10 sm:p-20 overflow-y-auto max-h-[80vh] lg:max-h-none custom-scrollbar">
                 <div className="flex justify-between items-start mb-12">
                    <div className="space-y-2">
                       <span className="text-[10px] font-black text-cyan-600 uppercase tracking-[0.4em]">{activeTab} Section</span>
                 ...
                 <div className="space-y-12">
                       ...
                       <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
                          {selectedProduct.sizes?.map(size => (
                            <button 
                              ...
                              className={`py-5 rounded-2xl text-[12px] font-black transition-all ${selSize === size ? 'bg-cyan-500 text-[#001a1a] shadow-lg shadow-cyan-500/20' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 border border-slate-100'}`}
                            >
                              {size}
                            </button>
                          ))}
                    ...
                    <div className="pt-8 flex flex-col sm:flex-row items-center gap-8">
                       ...
                       <button 
                         ...
                         className={`w-full sm:w-auto px-16 py-7 rounded-[2.5rem] font-black uppercase tracking-widest text-sm transition-all flex items-center justify-center gap-4 ${isAddDisabled ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-cyan-500 text-[#001a1a] shadow-2xl hover:bg-cyan-400 hover:scale-105 active:scale-95'}`}
                       >
